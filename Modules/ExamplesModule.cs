...
namespace DSPlus.Examples
{
    ...
    public class ExampleInteractiveCommands : BaseCommandModule
    {
        ...
        public async Task WaitForTyping(CommandContext ctx)
        {
            ...
            var chn = await interactivity.WaitForUserTypingAsync(ctx.User, TimeSpan.FromSeconds(60));
            if (!chn.TimedOut)
            {
                // got 'em
                await ctx.RespondAsync($"{ctx.User.Mention}, you typed in {chn.Result.Channel.Mention}!");
            }
            else
            {
